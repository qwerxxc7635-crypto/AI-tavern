...
- `AppInstanceLock` port由跨平台文件锁adapter实现；同一数据库的启动备份、恢复、永久删除与导入共享操作锁，独立Store和独立进程不能同时进入破坏性窗口。
- 桌面composition root在数据库打开前非阻塞取得全生命周期实例锁；第二实例无法进入应用运行期，也不会等待后继续操作同一数据库。
- 永久删除和覆盖导入保持同一数据库连接：锁内先检查目标并记录`PRAGMA data_version`，创建一致性备份，再取得`BEGIN IMMEDIATE`并重新核对版本和目标。备份后任何其他连接的提交都会取消操作，不删除或覆盖最新状态。
