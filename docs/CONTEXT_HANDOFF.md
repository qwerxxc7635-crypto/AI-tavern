...
- 分支：`main`
- 最近完成任务：`M3-T03 建立Prompt目录与版本机制`
- 已完成里程碑：M0、M1、M2
- 当前任务：`M3-T03` 已验收，准备提交
- 下一任务：`M3-T04 实现FakeAIProvider`
...
- 15类Prompt/历史完整，system与结构化输出能力降级测试通过，散落扫描0命中。
- `pnpm check`：通过；Vitest 154项、Node SQLite 7项通过。
...
3. 完成并提交当前 `M3-T03` 变更。
4. 从 `M3-T04` 实现覆盖15类任务的确定性FakeAIProvider。
