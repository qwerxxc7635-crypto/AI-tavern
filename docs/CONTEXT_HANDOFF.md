...
- 分支：`main`
- 最近完成任务：`M2-T08 实现pending_ai_requests`
- 已完成里程碑：M0、M1；M2已完成 `M2-T01` 至 `M2-T08`
- 当前任务：`M2-T08` 已验收，准备提交
- 下一任务：`M2-T09 实现数据库启动检查和迁移框架`
...
- 同一幂等键重复结算只产生一件奖励；错误码和重试次数可从SQLite恢复。
- `pnpm check`：通过；Vitest 112项、Node迁移3项通过。
...
3. 完成并提交当前 `M2-T08` 变更。
4. 从 `M2-T09` 实现数据库版本检查、迁移执行、完整性检查和失败保护。
