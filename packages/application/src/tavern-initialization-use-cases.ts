  ...
  private async generateValidated(
    ...
    try {
      ...
      this.generations.complete(command.generationRecordId, {
        ...
        validationError: generationError('PROVIDER_FAILURE', 'Tavern provider request failed'),
        completedAt: this.now(),
      });
      this.fail(command, 'PROVIDER_FAILURE', 'Tavern provider request failed', true);
      throw new AIOrchestrationError('PROVIDER_FAILURE', 'Tavern provider request failed', {
        cause: error,
      });
    }
    ...
    if (!validated.ok) {
      ...
      this.fail(command, validated.error.code, 'Tavern output validation failed', true);
      throw new AIOrchestrationError(validated.error.code, 'Tavern output validation failed');
    }
    this.generations.complete(command.generationRecordId, {
      rawResponseText: raw,
