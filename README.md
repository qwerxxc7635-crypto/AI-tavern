...
项目准备里程碑 M0、领域模型里程碑 M1、持久化里程碑 M2、AI基础设施里程碑 M3 和应用用例里程碑 M4 已完成。Windows Tauri 2客户端可启动，应用壳可在酒馆、任务、冒险、角色、档案和设置间导航；存档首页支持在系统应用数据目录的SQLite中新建、继续和归档存档。世界创建页可用基础选项和可选构想调用Fake Provider并确认世界；车卡页支持基础资料、10点属性分配、六选二特质、背景与程序控制的初始装备，完成后进入酒馆生成入口。酒馆及后续业务页面、真实Provider仍未实现。
...
应用默认进入存档首页，未完成的世界或车卡存档会返回对应创建页；六个游戏入口中的后续业务页面仍按任务顺序接入，下一项为M5-T06酒馆页面。
