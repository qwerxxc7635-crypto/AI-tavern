...
项目准备里程碑 M0、领域模型里程碑 M1、持久化里程碑 M2、AI基础设施里程碑 M3 和应用用例里程碑 M4 已完成。Windows Tauri 2客户端可启动，应用壳可在酒馆、任务、冒险、角色、档案和设置间导航；存档首页支持在系统应用数据目录的SQLite中新建、继续和归档存档，并显示最后游玩时间。世界创建及后续业务页面、真实Provider仍未实现。
...
应用默认进入存档首页；六个游戏入口仍是明确标注的页面骨架，将从M5-T04开始按任务顺序接入业务用例。
