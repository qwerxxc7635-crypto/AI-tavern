...
项目准备里程碑 M0 和领域模型里程碑 M1 已完成。pnpm workspace 已识别 Windows、iOS 和八个共享 package，Cargo workspace 已识别 `ember-native-bridge`；严格 TypeScript、ESLint、Prettier、Vitest、Rust fmt/Clippy 和基础 CI 已建立。共享协议及D20、关系、时钟规则已实现；SQLite ER模型、首版迁移，以及Campaign、世界和角色Repository已完成。其余Repository和应用功能仍未实现。
